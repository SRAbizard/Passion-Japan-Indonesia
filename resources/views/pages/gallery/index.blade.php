...
{{-- Category tab strip — only renders categories that have items --}}
@php $totalAll = array_sum($counts); @endphp
@if($totalAll > 0)
    ...
    <div class="flex flex-wrap items-center gap-2 justify-center">
        <a href="{{ route('gallery.index') }}"
           class="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition
               @if(! $activeCategory) bg-brand-600 text-white shadow-lg shadow-brand-900/30
               @else bg-surface-800/60 text-surface-300 hover:bg-surface-700 @endif">
            {{ __('All') }}
            <span class="text-xs opacity-75">{{ $totalAll }}</span>
        </a>
        @foreach(\App\Models\Gallery::CATEGORIES as $key => $label)
            ...
            @if($count > 0)
                <a href="{{ route('gallery.index', ['category' => $key]) }}"
                   class="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition
                       @if($activeCategory === $key) bg-brand-600 text-white shadow-lg shadow-brand-900/30
                       @else bg-surface-800/60 text-surface-300 hover:bg-surface-700 @endif">
                    {{ __($label) }}
