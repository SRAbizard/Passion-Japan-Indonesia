            ...
            @foreach($galleryPreview as $g)
                <a href="{{ route('gallery.index') }}" wire:navigate
                   class="group relative aspect-square overflow-hidden rounded-2xl bg-surface-900 border border-surface-800 hover:border-brand-500/50 transition">
                    @if($g->thumbnail_url)
                        <img src="{{ $g->thumbnail_url }}" alt="{{ $g->t('title') ?? '' }}" loading="lazy"
                    ...
                    @endif
                    @if($g->type !== 'image')
                        <span class="absolute top-3 right-3 inline-flex h-8 w-8 items-center justify-center rounded-full bg-brand-600/90 text-white shadow-lg">
